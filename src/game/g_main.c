...
Warn a specified admin.
=================
*/
void QDECL G_AdminPrintf(gentity_t *ent, const char *fmt, ... )
{
  va_list argptr;
  char    string[ 1024 ];	
  gentity_t   *tempent;
  ...
  va_end( argptr );
    ...
    {
       trap_SendServerCommand(ent-g_entities,va( "print \"^6[Admin]^7 %s\"", string) ); 
    }
    }
  else
  {
    char m2[ MAX_STRING_CHARS ];
    G_DecolorString( string, m2 );
    G_Printf( m2 );
  }
//   G_LogPrintf("%s",string);
